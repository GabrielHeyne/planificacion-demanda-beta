...
# --- Cargar demanda limpia desde sesión o archivo ---
@st.cache_data
def cargar_demanda():
    if 'demanda_limpia' in st.session_state:
        return st.session_state['demanda_limpia']
    elif os.path.exists("data/demanda_limpia.xlsx"):
        return pd.read_excel("data/demanda_limpia.xlsx")
    else:
        return pd.DataFrame()
...
# Asegúrate de que la columna de fechas sea tipo datetime para la correcta agrupación
...
# Filtrar solo los datos dentro del rango de fechas seleccionado (sin afectar el filtro por SKU)
df_quiebre_top = df_quiebre_top[(df_quiebre_top['fecha'] >= fecha_inicio) & (df_quiebre_top['fecha'] <= fecha_fin)]
...
# Convertir las unidades perdidas a enteros para visualización
